...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager) {
    this.errorListener = eventManager.subscribe('userAccountManagementApp.error', (response: JhiEventWithContent<AlertError>) => {
    ...
    this.httpErrorListener = eventManager.subscribe(
      'userAccountManagementApp.httpError',
      (response: JhiEventWithContent<HttpErrorResponse>) => {
        ...
        switch (httpErrorResponse.status) {
          ...
          case 404:
            this.addErrorAlert('UserId Not found');
            break;
